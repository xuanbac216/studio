	...
	jQuery.fn.fullwidthAudioPlayer = function(arg) {
		...
		function _init(elem) {
			...
			if(options.layout == "fullwidth") {
				$wrapper.addClass('fap-fullwidth').css({background: options.wrapperColor, '': options.strokeColor});
			}
			else if(options.layout == "boxed") {
				$wrapper.addClass('fap-boxed');
				$main.css({background: options.wrapperColor, '': options.strokeColor});
			}
			...
			options.wrapperPosition == 'top' ? $main.children('#fap-wrapper-switcher').css({'bottom': -17}) : $main.children('#fap-wrapper-switcher').css({'top': -17});
		...
		function _buildPlayer() {
			...
			if(options.socials) {
				$metaWrapper.children('#fap-track-info').append('<p id="fap-social-links"><a data-music="" target="_blank" style="color: '+options.metaColor+';">'+options.facebookText+'</a><a data-music="" target="_blank" style="color: '+options.metaColor+';">'+options.twitterText+'</a><a data-music="" target="_blank" style="color: '+options.metaColor+';">'+options.downloadText+'</a><a data-music="" target="_blank" class="fap-soundcloud-link"></a></p>');
			}
			...
			$mainControls.append('<a data-music="#" id="fap-previous" class="fa fa-backward" style="color: '+options.mainColor+';"></a>').children('#fap-previous').click(function() {
				$.fullwidthAudioPlayer.previous();
				return false;
			});

			//append play/pause button
			$mainControls.append('<a data-music="#" id="fap-play-pause" class="fa fa-play" style="color: '+options.mainColor+';"></a>').children('#fap-play-pause').click(function() {
				$.fullwidthAudioPlayer.toggle();
				return false;
			});

			//append next button
			$mainControls.append('<a data-music="#" id="fap-next" class="fa fa-forward" style="color: '+options.mainColor+';"></a>').children('#fap-next').click(function() {
			...
			if(options.popup && !isPopupWin) {
				...
				$playlistControls.append('<a data-music="#" id="fap-player-popup" class="fa fa-caret-square-o-up" style="color:'+options.mainColor+';" title="Pop-up Player"></a>')
			...
			if(options.shuffle) {
				...
				$playlistControls.append('<a data-music="#" id="fap-playlist-shuffle" class="fa fa-random" style="color:'+options.mainColor+';" title="Shuffle"></a>')
			...
			if(options.playlist) {
				...
				if(!isPopupWin) {
					//playlist switcher
					$playlistControls.append('<a data-music="#" id="fap-playlist-toggle" class="fa fa-list" style="color:'+options.mainColor+';" title="Playlist"></a>')
		...
		function _createPlaylistTrack(cover, title) {
            ...
            var coverDom = cover ? '<img src="'+cover+'" style="border: 0px solid '+options.mainColor+';" />' : '<div class="fap-cover-replace-small" style="background: '+options.wrapperColor+'; border: 0px solid '+options.mainColor+';"></div>';
...
window[e+"CancelRequestAnimationFrame"])});var E=window.MutationObserver||window.WebKitMutationObserver||!1,K={zindex:"auto",cursoropacitymin:0,cursoropacitymax:1,cursorcolor:"#424242",cursorwidth:"5px",cursorborder:"0px solid #fff",cursorborderradius:"5px",scrollspeed:60,mousescrollstep:24,touchbehavior:!1,hwacceleration:!0,usetransition:!0,boxzoom:!1,dblclickzoom:!0,gesturezoom:!0,grabcursorenabled:!0,autohidemode:!0,background:"",iframeautoresize:!0,cursorminheight:32,preservenativescrolling:!0,
