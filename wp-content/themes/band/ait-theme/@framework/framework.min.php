...
function
overrideCptMetaboxConfig($path,$filename){$paths=aitPaths();if(file_exists("{$paths->dir->cptsMetaboxesConfig}/$filename")){return"{$paths->dir->cptsMetaboxesConfig}/$filename";}else{return$path;}}}class
AitLangs{protected
...
AitAdminBar{static
function
register(){add_action("wp_ajax_toggleDevMode",array(__CLASS__,"ajaxToggleDevMode"));add_action("wp_ajax_emptyThemeCacheDir",array(__CLASS__,"ajaxEmptyThemeCacheDir"));add_action("wp_ajax_emptyWPThumbCacheDir",array(__CLASS__,"ajaxEmptyWPThumbCacheDir"));add_action('admin_bar_menu',array(__CLASS__,'generateThemeAdminMenu'),31);add_action('admin_bar_menu',array(__CLASS__,'generateDevMenu'),1999);add_action('admin_bar_menu',array(__CLASS__,'enhanceDefaultBar'),21);add_action('admin_bar_menu',array(__CLASS__,'addPageBuilderLink'),100);add_action('admin_head',array(__CLASS__,'cssAndJs'));add_action('wp_head',array(__CLASS__,'cssAndJs'));}static
function
generateThemeAdminMenu($wp_admin_bar){$adminMenuItems=aitConfig()->getAdminConfig('pages');if(!empty($adminMenuItems)){$wp_admin_bar->add_node(array('id'=>'ait-admin-menu','title'=>'<span class="ab-icon"></span><span class="ab-label">'.__('Theme Admin','ait-admin').'</span>','href'=>'#','meta'=>array('class'=>'ait-admin-menu')));$t=aitConfig()->getFullConfig('theme');foreach($adminMenuItems
...
isOptionsSection($value){return((is_string($value)and$value=='section')or($value
instanceof
NNeonEntity));}protected
function
getOptionsSection($value,$hidden=false){$return=new
stdClass;$return->title=false;$return->help=false;$return->id=false;$return->hidden=$hidden;$return->allAreAdvanced=false;$return->capabilities=false;if($value===true)return$return;if($value
instanceof
NNeonEntity){if(isset($value->attributes['help'])and!empty($value->attributes['help'])){$return->help=$value->attributes['help'];}if(isset($value->attributes['title'])and!empty($value->attributes['title'])){$return->title=$value->attributes['title'];}if(isset($value->attributes['id'])and!empty($value->attributes['id'])){$return->id=$value->attributes['id'];}if(isset($value->attributes['advanced'])and!empty($value->attributes['advanced'])and$value->attributes['advanced']===true){$return->allAreAdvanced=true;}if(isset($value->attributes['capabilities'])and!empty($value->attributes['capabilities'])and$value->attributes['capabilities']===true){$return->capabilities=true;}}return$return;}function
...
function
init(){add_filter('post_gallery',array(__CLASS__,'postGallery'),10,2);add_filter('tiny_mce_before_init',array(__CLASS__,'tinymceUnhideKitchensink'));add_filter('posts_join',array(__CLASS__,'joinPostsMetadataToEnableSearchingInElements'));add_filter('posts_search',array(__CLASS__,'enableSearchingInElements'),10,2);add_filter('posts_request',array(__CLASS__,'showOnlyDistinctPostSearchResults'));}static
function
...
function
showOnlyDistinctPostSearchResults($query){if(strstr($query,'DISTINCT')===false){$query=str_replace('SELECT','SELECT DISTINCT',$query);}return$query;}private
static
function
...
AitSkeletonUpgrade2812;$this->errors=$upgrade->execute();}$this->addAdminNotices();do_action('ait-skeleton-upgrade',$this);}protected
function
addAdminNotices(){if(!empty($this->errors)){add_action('admin_notices',array($this,'adminNotices'));}}function
themeUpgrade(){do_action('ait-theme-upgrade',$this);}function
