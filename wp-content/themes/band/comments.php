			...
			<article {!$comment->htmlId} class="comment-article">
				...
				<header class="comment-meta">
					<div class="comment-author vcard">
						{!$comment->author->avatar(60)} {* edit avatar size to 60x60 *}
						<cite class="fn">{!$comment->author->link}</cite>
					</div><!-- .comment-meta -->

{* moved to .comments-footer
...
{* Added .comments-footer *}
				<footer class="comments-footer">
					...
					<a href="{$comment->url}" class="comment-time"><time datetime="{$comment->time('c')}">{_x '%1$s at %2$s', '1: date, 2: time'|printf: $comment->date, $comment->time}</time></a>
